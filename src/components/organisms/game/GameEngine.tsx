...
export const GameEngine: FC<ClassNameProps> = ({className}) => {
    ...
    // Calculate game speed based on level only, prevents unexpected speed changes
    const gameSpeed = (() => {
        if (running && speed < 50) {
            // Prevent speed from dropping too low
            return Math.max(50, 500 - level * 20);
        }
        return speed;
    })();
    ...
    return (
        <div
            className={classNames(
                className,
                'flex relative overflow-visible rounded-lg desktop:p-4 w-full h-full min-h-[500px] pb-12',
                {
        ...
        >
            <GameGrid className="w-full h-full" />
            {running && <GameMusic />}
            <GameSoundTracks />
            {starting && !loaded && (
                <GamePreloader
