...
import {GameStatus, GameTool, GameToolType, NextTetrominos} from './game-model';
...
export namespace GameSelectors {
    ...
    export const speed = createSelector(game, (game) => {
        ...
        if (game.score >= 90 && game.score <= 110) {
            console.log(`Game speed calculation: level=${game.level}, score=${game.score}, initialSpeedFactor=${game.initialSpeedFactor}`);
        }
        
        // Use initialSpeedFactor if available to maintain consistent speed progression
        // This prevents sudden speed changes when tools are unlocked
        const speedFactor = game.initialSpeedFactor || (game.level * 20);
        const speed = 500 - speedFactor;
    ...
    /**
     * Selects a specific game tool
     */
    export const tool = (type: GameToolType) => 
        createSelector(tools, (tools) => tools[type]);
    
    /**
     * Selects all available tools
     */
    export const availableTools = createSelector(
        tools,
        (tools): GameTool[] => Object.values(tools).filter(tool => tool.available)
    );
    
    /**
     * Checks if a saved game exists
     */
    export const hasSavedGame = createSelector(
        game,
        (game) => game.savedGame !== undefined
    );
}
